...
/**
 * Next.js API route that proxies email verification request to the backend
...
export async function POST(request: NextRequest) {
  try {
    ...
    let backendUrl = getApiUrl('api/auth/verify-email/request')
    ...
    console.log('[verify-email/request] Proxying to backend:', backendUrl)
    ...
    console.log('[verify-email/request] Backend request headers:', {
    ...
    console.log('[verify-email/request] Backend response:', {
    ...
    console.error('[verify-email/request] Proxy error:', error)
    const errorMessage = error instanceof Error ? error.message : 'Unknown error'
    return NextResponse.json(
      { 
        detail: `Failed to request verification email: ${errorMessage}`,
