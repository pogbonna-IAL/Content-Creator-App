            ...
            {parsedContent ? (
              ...
              <div className="space-y-4">
                {parsedContent.linkedin_post && (
                  <div className="bg-gradient-to-br from-blue-900/20 to-blue-800/10 rounded-lg p-5 border border-blue-500/30 shadow-lg">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <span className="ml-auto text-xs text-gray-400 bg-blue-900/30 px-2 py-1 rounded">Professional Network</span>
                    </div>
                    <div className="bg-dark-card/50 rounded-lg p-4 border border-blue-500/20">
                      <SocialMediaFormatter 
                        platform="linkedin" 
                        content={parsedContent.linkedin_post}
                        cta={parsedContent.cta}
                      />
                    </div>
                    <div className="flex items-center justify-between mt-3 text-xs text-gray-400">
                      <span>{parsedContent.linkedin_post.length} characters</span>
                      <span className="text-blue-400">✓ Optimized for B2B audience</span>
                    </div>
                  </div>
                )}
                
                {parsedContent.twitter_post && (
                  <div className="bg-gradient-to-br from-sky-900/20 to-sky-800/10 rounded-lg p-5 border border-sky-500/30 shadow-lg">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <span className="ml-auto text-xs text-gray-400 bg-sky-900/30 px-2 py-1 rounded">Short-form</span>
                    </div>
                    <div className="bg-dark-card/50 rounded-lg p-4 border border-sky-500/20">
                      <SocialMediaFormatter 
                        platform="twitter" 
                        content={parsedContent.twitter_post}
                        cta={parsedContent.cta}
                      />
                    </div>
                    <div className="flex items-center justify-between mt-3">
                      <div className={`text-xs font-medium ${parsedContent.twitter_post.length > 250 ? 'text-yellow-400' : parsedContent.twitter_post.length > 260 ? 'text-red-400' : 'text-gray-400'}`}>
                        {parsedContent.twitter_post.length} / 280 characters
                      </div>
                      <span className="text-xs text-sky-400">✓ Optimized for quick engagement</span>
                    </div>
                  </div>
                )}
                
                {parsedContent.facebook_post && (
                  <div className="bg-gradient-to-br from-blue-900/20 to-indigo-800/10 rounded-lg p-5 border border-blue-500/30 shadow-lg">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <span className="ml-auto text-xs text-gray-400 bg-blue-900/30 px-2 py-1 rounded">Community</span>
                    </div>
                    <div className="bg-dark-card/50 rounded-lg p-4 border border-blue-500/20">
                      <SocialMediaFormatter 
                        platform="facebook" 
                        content={parsedContent.facebook_post}
                        cta={parsedContent.cta}
                      />
                    </div>
                    <div className="flex items-center justify-between mt-3 text-xs text-gray-400">
                      <span>{parsedContent.facebook_post.length} characters</span>
                      <span className="text-blue-500">✓ Optimized for community engagement</span>
                    </div>
                  </div>
                )}
                
                {parsedContent.instagram_post && (
                  <div className="bg-gradient-to-br from-pink-900/20 via-purple-900/20 to-pink-800/10 rounded-lg p-5 border border-pink-500/30 shadow-lg">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <span className="ml-auto text-xs text-gray-400 bg-pink-900/30 px-2 py-1 rounded">Visual-first</span>
                    </div>
                    <div className="bg-dark-card/50 rounded-lg p-4 border border-pink-500/20">
                      <SocialMediaFormatter 
                        platform="instagram" 
                        content={parsedContent.instagram_post}
                        hashtags={parsedContent.hashtags}
                        cta={parsedContent.cta}
                      />
                    </div>
                    <div className="flex items-center justify-between mt-3 text-xs text-gray-400">
                      <span>{parsedContent.instagram_post.length} / 2200 characters</span>
                      <span className="text-pink-500">✓ Optimized for visual storytelling</span>
                    </div>
                  </div>
                )}
